...
const Feed = () => {
    ...
    const getFeed = async () => {
        if(feed) return;
        try {
            const userCards = await axios.get("http://localhost:7777/user/feed", { withCredentials: true } );
    ...
    return (
        <div className="flex justify-center mt-14">
           {feed && <div className="card bg-base-300 w-68 shadow-sm">
                <figure>
                    <img
                    src={feed[0]?.profileImageURL}
                    alt="Shoes" />
                </figure>
                <div className="card-body">
                    <h2 className="card-title">{feed[0]?.firstName} {feed[0]?.lastName}</h2>
                    <p>{feed[0]?.age} {feed[0]?.gender}</p>
                    <p>{feed[0]?.about}</p>
